...
class PandaDataset(Dataset):
    def __init__(self, root_path, df, level=2, num_patches=5, patch_size=256, num_trials=10, mode='train', use_mask=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.num_patches:
            ...
            regions = np.stack(np.where(~np.all(image == [255, 255, 255, 255], axis=-1)), axis=1)
            ...
            image = image.reshape(image.shape[0] // self.patch_size, self.patch_size, image.shape[1] // self.patch_size, self.patch_size, 4)
            image = image.transpose(0, 2, 1, 3, 4).reshape(-1, self.patch_size, self.patch_size, 4)
            #proportion_blank = np.mean(np.all(image == [255, 255, 255, 255], axis=3), axis=(1, 2))
            proportion_blank = np.mean((image - np.array([255, 255, 255, 255])) ** 2, axis=(1, 2, 3))


            if image.shape[0] < self.num_patches:
                image = np.pad(image, ((0, self.num_patches - image.shape[0]), (0, 0), (0, 0), (0, 0)))

            selected_patches = np.argsort(proportion_blank)[-self.num_patches:]#[:self.num_patches]
            ...
            if self.use_mask:
                ...
                if self.num_patches: # Apply the identical operations to the masks
                    ...
                    if mask.shape[0] < self.num_patches:
                        mask = np.pad(mask, ((0, self.num_patches - mask.shape[0]), (0, 0), (0, 0), (0, 0)))
